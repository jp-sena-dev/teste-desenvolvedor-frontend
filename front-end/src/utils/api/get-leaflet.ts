...
export const fetchLeaflets = async (query: Queryleaflet): Promise<FetchLeaflet> => {
  const URL = `http://localhost:3000/data?company=${query.company?.trim().toLocaleUpperCase() || ''}&name=${query.name?.trim().toLocaleUpperCase() || ''}&_sort=published_at&_page=${query.page}&_per_page=${itemPerPage}`;
